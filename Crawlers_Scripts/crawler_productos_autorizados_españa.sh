#!/usr/bin/env bash
LOGFILE="/home/catalin/Escritorio/TFG/TFG/crawlerlog.log"
...
if [ $? -eq 0 ]; then
    FECHAEJECUCION="$(date +%Y_%m_%d-%H_%M)"
...
/home/catalin/Escritorio/TFG/HADOOP/hadoop-2.8.0/bin/hadoop fs -put $FICHERO_CONVERTIDO /user/TFG/Productos_fitosanitarios/España/Productos_autorizados
if [ $? -eq 0 ]; then
    EJECUCION=$?
    FECHAEJECUCION="$(date +%Y_%m_%d-%H_%M)"
    echo ${FECHAEJECUCION}": Se ha copiado el fichero en Hadoop. " >> ${LOGFILE}
else
    echo ${FECHAEJECUCION}": ERROR: No se ha copiado el fichero en Hadoop. Código: "${EJECUCION} >> ${LOGFILE}
fi
