...
function func1() {
    ...
    if [ $? -eq 0 ]; then
        ...
        libreoffice --convert-to xls  --outdir $(getProperty crudo_espanya_autorizados_5) $FICHERO_DESCARGADO 2>> ${LOGFILE}
        if [ $? -eq 0 ]; then
            FECHAEJECUCION="$(date)"
            echo ${FECHAEJECUCION}": Se ha convertido el fichero viejo" >> ${LOGFILE}
            FICHERO_CONVERTIDO="$(echo $(getProperty crudo_espanya_autorizados_5)"ListadoProductos_"${FECHA}".xls")"
